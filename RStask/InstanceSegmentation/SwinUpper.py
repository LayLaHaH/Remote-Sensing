...
class SwinInstance:
    def __init__(self, device):
        ...
        self.device = device
        try:
            trained = torch.load('./checkpoints/last_swint_upernet_finetune.pth',map_location=torch.device('cpu'))
        except:
            trained = torch.load('D:/fifthYear/5th_year_project/Remote-Sensing-ChatGPT-main/checkpoints/last_swint_upernet_finetune.pth',map_location=torch.device('cpu'))
