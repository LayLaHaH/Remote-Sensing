...
class HRNet48(nn.Module):
    def __init__(self,device):
        ...
        self.device = device
        try:
            trained = torch.load('./checkpoints/HRNET_LoveDA_best.pth',map_location=torch.device('cpu'))
        except:
            trained = torch.load('.D:/fifthYear/5th_year_project/Remote-Sensing-ChatGPT-main/checkpoints/HRNET_LoveDA_best.pth',map_location=torch.device('cpu'))
